...
# Checks a DLL for dynamic dependencies on
# common C/C++ runtime libraries using dumpbin.exe.
def check_for_static_runtime(dll_path: Path, should_link_statically: bool) -> bool:
    ...
    dependency_regex = re.compile(r"^\s+([a-zA-Z0-9_.\-]+\.dll)$")
    for line in dumpbin_output_text.splitlines():
        match = dependency_regex.match(line)  # Match against the original line content
        if match:
            dependencies.add(match.group(1))  # Group 1 is the DLL name
